...
single_day = date(year=2016,month=6,day=18)
print(single_day)
...
surv = {"Bmagnitude":Btotal, "Frequency":survey_freq, "Epoch":survey_epoch}

# get all gyro's/flhr

mag_dat = day_files.magnetic_data
...
PSD_folder = '/data/emfisis_burst/wip/rablack75/rablack75/SLURM_out'
# What is the date and time of this burst? 
FFT_file_path = PSD_folder + '/' +year+'/'+month + '/output/' + day
# FFT Full filepath
...
for FFT_Kletzing, FFT_sliding in zip(FFT_files_K, FFT_files_s):
    ...
    fft_datetime = FFT_K["BurstDatetime"][...]
    dataset["timestamp"].loc[k] = fft_datetime
     #time_thresh = datetime(year=2018, month=1, day = 21, hour=16, minute=14, second=5)
    time_min = datetime(year=2016,month=6,day=18,hour=8,minute=0,second=1)
    ...
    fft_reduced_s = fa.BackReduction(FFT_s["PSD"], 'B', 's')#FFT_s["PSD"]
...
if not os.path.exists(f'/data/hpcflash/users/rablack75/power_netCDFs/bugss/{year}/{month}'): 
    # if the directory is not present  
    # then create it. 
    os.makedirs(f'/data/hpcflash/users/rablack75/power_netCDFs/bugss/{year}/{month}')

dataset.to_netcdf(f'/data/hpcflash/users/rablack75/power_netCDFs/bugss/{year}/{month}/power_{date_string}.nc')
